...
class TestGameService:
    ...
    def test_make_move_out_of_bounds(self):
        ...
        # Test valid out of bounds coordinates (but within value object constraints)
        valid_but_out_of_bounds = [
            GridCoordinate(3, 0),  # Row too high  
            GridCoordinate(0, 3),  # Column too high
            GridCoordinate(5, 5),  # Both too high
        ]
        
        for coord in valid_but_out_of_bounds:
            success, message = service.make_move(coord)
            assert success is False
            
        # Test that negative coordinates are rejected by GridCoordinate validation
